...
export default function PackEmotion({
  contentType,
  contentUrl,
  contentMetadata,
  travelerName,
  ...
  const audioDuration =
    typeof contentMetadata?.duration === 'number'
  ...
  return (
    <main
      ...
      style={{
        background:
          'linear-gradient(135deg, #7C3AED 0%, #A855F7 30%, #EC4899 70%, #F472B6 100%)',
      }}
    >
            ...
            <div className="w-24 h-16 rounded-t-lg bg-white/30 backdrop-blur-sm border-2 border-white/40 flex items-center justify-center">
              <Heart
                className={`w-8 h-8 transition-colors duration-500 ${
                  isOpen ? 'text-pink-500' : 'text-white/60'
                }`}
              />
            </div>
            <motion.div
              ...
              {isText && message && (
                <div className="bg-white/15 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <div className="bg-white/10 rounded-xl p-5 relative">
                    ...
                    <p
                      className="text-white text-lg leading-relaxed font-medium italic pt-4 pb-2 px-2"
