...
export default function PackImmobilier({
  reference,
  contentMetadata,
  travelerName,
}: PackImmobilierProps) {
  ...
  const nextImage = () =>
    setCurrentImage((i) => (i + 1) % (images.length || 1));
  const prevImage = () =>
    setCurrentImage((i) => (i - 1 + (images.length || 1)) % (images.length || 1));
          ...
          {isSubmitted ? (
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-600">
                {"L'agent vous recontactera bientôt."}
              </p>
            </div>
          ) : (
